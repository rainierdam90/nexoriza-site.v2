...
/** Only SET EX NX + GETDEL: compatible with Redis and the existing private
 * pairing store. Senders retry identical sequence IDs until acknowledged.
 * The TV caches replies, so a retry cannot apply a mutation twice.
...
async function command(args: string[]): Promise<unknown> {
  ...
  if (op === 'SET') {
    ...
    if (memory.size >= 200) throw new Error('Development relay full')
...
export async function relay(id: string, op: 'create' | 'get' | 'put' | 'delete', role: string, hash: string, payload: object = {}): Promise<Result> {
  ...
  if (op === 'create') {
    await command(['SET', key + ':probe', '1', 'EX', '60', 'NX']); await command(['GETDEL', key + ':probe'])
    return { status: 204 }
  }
  ...
  const result = await command(['SET', key + (role === 'phone' ? ':command' : ':reply'), JSON.stringify(payload), 'EX', String(MANAGE_TTL), 'NX'])
  return { status: result === 'OK' ? 204 : 409 }
}
